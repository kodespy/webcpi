...
Template.listaPreguntas.helpers({
  esAdmin: function() {
    return Blaze._globalHelpers.isAdmin();
  },
	preguntas: function () {
    var admin = Blaze._globalHelpers.isAdmin();
    if (admin==true) {
      return Preguntas.find({}, {sort: {fechaM: -1}})
    }
		var materias = [];
		Materias.find({idProfesor: Blaze._globalHelpers.obtenerIdProfesor()}).forEach( function(item) { 
...
Template.agregarPregunta.helpers({
	materias: function () {
		return Materias.find({idProfesor: Blaze._globalHelpers.obtenerIdProfesor()});
	},
